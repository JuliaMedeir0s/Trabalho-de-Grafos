  ...
  handleClick(e) {
    ...
    if (this.selected.length === 1) {
      ...
      const confirmLogic = () => {
        ...
        this.drawGraph();
        if (this.distDiv) this.distDiv.textContent = `Total: ${result.total > 0 || (result.path.length > 0 && result.total === 0) ? result.total.toFixed(2) : 'N/A'} m.`;
        const caminhoDiv = document.getElementById("caminho");
        if (caminhoDiv) {
          if (result.path && result.path.length > 0) {
            ...
            if (this.distDiv) this.distDiv.textContent = `Não foi possível encontrar um caminho.`;
          }
        }
      };
